...
class CoiledAltarMechanicsTest(unittest.TestCase):
    ...
    def test_gravebound_failures_require_gravebound_damage_kill(self):
        ...
        deaths = [
            ...
            # 有墓缚但死于其他技能 → 不计入
            {"timestamp": 95_000, "type": "death", "targetID": 11, "killingAbilityGameID": 1},
        ]
        damage = [
            {"timestamp": 94_500, "type": "damage", "abilityGameID": 1297906, "targetID": 11, "amount": 1},
        ]
        # 11 虽 killingAbility 不是墓缚，但临死前有墓缚伤害 → 计入
        deaths_b = [
            {"timestamp": 70_000, "type": "death", "targetID": 10, "killingAbilityGameID": 1308330},
            {"timestamp": 95_000, "type": "death", "targetID": 11, "killingAbilityGameID": 1},
        ]
        result = coiledaltar.analyze_gravebound_failures(
            fight, debuffs, deaths_b, actor_map, players, damage_events=damage,
        )
        self.assertEqual(len(result["failures"]), 2)
        by_name = {row["player"]: row for row in result["failures"]}
        self.assertTrue(by_name["Tank"]["killedByGraveboundDamage"])
        self.assertEqual(by_name["Tank"]["deathAbilityID"], 1308330)
        self.assertTrue(by_name["Dps"]["killedByGraveboundDamage"])
        self.assertEqual(by_name["Dps"]["deathAbilityID"], 1297906)
        ...
        self.assertTrue(result["applications"][0]["shieldBroken"])
