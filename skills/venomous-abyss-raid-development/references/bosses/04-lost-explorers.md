...
- The normal loop is two stacked targets plus one moving target. Multi-dotting is valuable, but bringing both other bosses into Nama's 联合防御 causes 99% damage reduction.
...
- Entity coordinates follow the Crown/Alleria audit rule: retain `sourceInstance`, then read the source actor's coordinate-bearing Cast/Damage event with `includeResources`. For Mythic crates, `Useless Junk` game ID `272110` exposes its activated position on `1310028`; silent spawned instances do not carry coordinates in WCL and must not be invented.
- `Bouncy Mushroom` game ID `268045` casts `1299855` with its own position when spawned. The cast locates the entity but does not identify the activator; activation is the first `1299854` apply from that same `sourceInstance`. Player position at that timestamp is an independent confirmation, not a replacement for the direct first-target event.
